...
class SearchNode:
    ...
    def extract_solution(self):
        ...
        stateseq = []
        final_node = self
        stateseq.append(self.state)
        while self.parent is not None:
            solution.append(self.action)
            stateseq.append(self.parent.state)
            self = self.parent
        solution.reverse()
        if VERBOSE:
            stateseq.reverse()
...
def make_child_node(parent_node, action, state):
    ...
    The g-value is set to the parents g-value + cost of action.
    """
    
    sda_cost = action.cost 
    #print 'Before accounting for conditinal costs the action.cost is ', action.cost
    
    if state.sdac_optimization : 
        #sda_cost += state.optimal_value
        sda_cost += action.end_plan_constant * state.optimal_value
        #print("Action:", action.name,"Adding optimal value: ", state.optimal_value, "sda cost: ", sda_cost) 
    else :
        assert len(action.conditional_costs) == 0 # c-costs are not supported; should use objfun mech.
        for c in action.conditional_costs: 
            if c.evaluate(parent_node.state): #evaluate the condition in the state
            #print("Added Cost:",c.ccost)
                sda_cost += c.ccost
    #print("Action:", action.name,"Adding optimal value: ", state.optimal_value, "sda cost: ", sda_cost)
    #print "Parent node cost: ", parent_node.g, " sda_cost: ", sda_cost, "The cost of the node: ", parent_node.g + sda_cost
    return SearchNode(state, parent_node, action, parent_node.g + sda_cost )
