...
namespace Sgt
{
    ...
    bool PsrSolver::acRectCheck(json* msg)
    {
        ...
        (*msg)["objective"] = -powerModel_->_model->_opt; // IPOPT solver converts maximise into minimise, hence * -1.
        ...
        {
            Sgt::json fedJsn;
            for (const auto& biPair : busInfos())
            {
                const auto& id = biPair.first;
                const auto& bi = biPair.second;
                double fedVal = 0.0;
                if (bi.isActive())
                {
                    auto& modBus = bi.modelBus<AcRectModel>(); 
                    fedVal = modBus.fed.get_value();
                }
                fedJsn[id] = fedVal;
            }
            (*msg)["fed"] = fedJsn;
    ...
    template<typename TMod> void PsrSolver::setMaxLoadObjective()
    {
        ...
        for (auto& busInf : busInfos_)
        {
            ...
            *obj += max(1e-5, busInf.second.ptBus().pl()) * modBus.fed;
        }
    ...
    template<typename TMod> void PsrSolver::addSpecialVoltageObjTerm()
    {
        ...
        for (auto& busInf : busInfos_)
        {
            ...
            auto& modBus = busInf.second.modelBus<TMod>();
            *powerModel_->_model->_obj += 1e-6 * (modBus.Vr + modBus.Vi);
        }
    }
