...
class HybridTask :
	...
	def __init__( self, task, lp, s0, primary_G, secondary_G = set(), sdac_optimization = False ) :
		...
		if self.prim_s0 is not None :
			self.s0 = self.initial_state = HybridState( self.prim_s0, 
                                                                    self.lp, 
                                                                    self.inactive_by_default.copy(),
                                                                    sdac_optimization = self.sdac_optimization )
		#assert self.s0.secondary is not None

	def set_initial_state( self, s ) :
		self.s0 = self.initial_state = HybridState( s, self.lp, 
                                                            self.inactive_by_default.copy(), 
                                                            sdac_optimization = self.sdac_optimization )
	...
	def compute_successor_state( self, s, action ) :
		...
		prim_succ.set_vec( action.effect )
                ...
                #print("Creating a hybrid state:", self.sdac_optimization)
		succ = HybridState( prim_succ, self.lp, self.inactive_by_default.copy(), sdac_optimization = self.sdac_optimization )
		succ.check_valid()
                
                if not succ.valid :
			return None
                
                #new stuff added for sdac
                if self.sdac_optimization : 
                    
                    s_p = get_primary(s).copy() #Not sure how is this supposed to work. Copied from check_secondary_precondition method. 
                    s_p.set_vec(action.prim_precs)
                    
                    #check_validity_additional_constraints( self, s, additional ) :
                    if action.sec_precs is None:
                            inactive = self.inactive_by_default
                    else:
                            inactive = self.inactive_by_default - action.sec_precs
		#logging.debug( 'Checking validity with {0} additional constraints'.format( len(additional)) )
                    try :
                            tmp = HybridState( s_p.primary, self.lp, inactive.copy(), sdac_optimization = self.sdac_optimization )
                    except AttributeError :
                            tmp = HybridState( s_p, self.lp, inactive.copy(), sdac_optimization = self.sdac_optimization )
		
                    #Add the objective function. 
                    #print("Action name:", action.name)
                    tmp.check_valid( a_optimize = action )
                    if not tmp.valid : 
                            return None
		
                    #Get the optimal value. 
                    #objective = tmp.lp.model.getObjective()
                    #objective = tmp.secondary.getObjective()
                    #objective_value = tmp.tmp_objective_value
                    #print("(In task.py) ObjectiveValue", objective_value)
                    succ.optimal_value = tmp.tmp_objective_value
                    #print("Storing value:", succ.optimal_value)
                    
                #end new stuff added for sdac
	...
	def get_successor_states( self, s ) :	
        ...
        def validate( self, plan ):
                ...
                for aname in plan:
                        #print( "====  STATE ", step, " ====" )
                        print( "====  STATE ", step, "; PLAN COST ", plan_cost," ====" )
                        ...
                        if self.sdac_optimization:
                                s_p = get_primary(state).copy()
                                s_p.set_vec(action.prim_precs)
                                if action.sec_precs is None:
                                        inactive = self.inactive_by_default
                                else:
                                        inactive = self.inactive_by_default - action.sec_precs
                                tmp = HybridState( s_p, self.lp, inactive.copy(), sdac_optimization = True )
                                #Add the objective function. 
                                #print("Action name:", action.name)
                                tmp.check_valid( a_optimize = action )
                                assert tmp.valid
                                step_cost = action.cost + tmp.tmp_objective_value
                        else:
                                step_cost = action.cost
                        print( "action cost is " + str( step_cost ) + "\n" )
	                ...
	                succ = HybridState( prim_succ, self.lp, self.inactive_by_default.copy() )
                        ...
                        # #find the cost of applying the action in state 
                        # sda_cost = action.cost 
                        # #print 'Before accounting for conditinal costs the action.cost is ', action.cost
                        # for c in action.conditional_costs: 
                        #     if c.evaluate(state): #evaluate the condition in the state
                        #         #print("Added Cost:",c.ccost)
                        #         sda_cost += c.ccost
                ...
                print( "====  STATE ", step, "; PLAN COST ", plan_cost, "; FINAL ====" )
                state.write(sys.stdout)
                ...
                if len(self.Gs) > 0:
                        ...
                        if self.check_secondary_goal( state ):
                                ...
                                return False
                print("Total plan cost: ", format(plan_cost))
