...
class HydraulicBlocksWorldTask( Task ) :
        ...
        def __init__( self, object_set, sdac = False ) :
        ...
        def __make_pickup_action_4( self, block, cylinder, lp = None ) :
			...
			( self.block_container[block].index, self.block_container_values[ 'none' ] ) ]
                ...
                return Action( name, prec, [], eff, objective_function = o_f, of_vname = of_varname )

        def __make_putdown_action_4( self, block, cylinder, lp = None ) :
        ...
        def __make_unstack_action_4( self, top_block, bot_block, cylinder, lp = None ) :
        ...
        def create_actions_4ops( self, the_lp = None ) :
                self.actions = []

                self.pickup_actions = []
                for b in xrange (self.objects.num_blocks()):
                        for c in xrange (self.objects.num_cylinders()):
                                self.pickup_actions.append(self.__make_pickup_action_4(b, c, lp = the_lp))
                ...
                for b in xrange (self.objects.num_blocks()):
                        for c in xrange (self.objects.num_cylinders()):
                                self.putdown_actions.append(self.__make_putdown_action_4(b, c, lp = the_lp))
                ...
                for b1 in xrange (self.objects.num_blocks()):
                        for b2 in xrange (self.objects.num_blocks()):
                                if b1 != b2:
                                        for c in xrange (self.objects.num_cylinders()):
                                                self.unstack_actions.append ( self.__make_unstack_action_4(b1, b2, c, lp = the_lp))
                ...
                for b1 in xrange (self.objects.num_blocks()):
                        for b2 in xrange (self.objects.num_blocks()):
                                if b1 != b2:
                                        for c in xrange (self.objects.num_cylinders()):
                                                self.stack_actions.append ( self.__make_stack_action_4(b1, b2, c, lp = the_lp))
