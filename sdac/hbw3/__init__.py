...
def	make_goal( instream, task ) :
	...
	return [ (x,v) for x,v in valuation.iteritems() ]

def 	create_prob_from_file( filename, total_amount_of_fluid, constrained = True, state_dependent_costs = False ) :
	...
	the_task = HydraulicBlocksWorldTask( task_objs, sdac = state_dependent_costs )
        if constrained:
	...
	the_task.create_actions_4ops(the_lp = the_LP)
	...
	return HybridTask( the_task, the_LP, s0, G, sdac_optimization = state_dependent_costs )	
